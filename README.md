...
A base historica deve conter uma linha por concurso e quinze colunas de dezenas sorteadas. A pagina oficial das Loterias CAIXA informa area de download de resultados da Lotofacil por ordem crescente. A base local usada em cada execucao deve ser registrada em `data/raw/` ou informada no objeto `config` do script R.
